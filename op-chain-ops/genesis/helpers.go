...
var (
	...
	l2PredeployNamespaceEcosystemAndIP = common.HexToAddress("0x1900000000000000000000000000000000000000")
	// BigL2PredeployNamespace represents the predeploy namespace as a big.Int
	BigL2PredeployNamespaceEcosystemAndIP = new(big.Int).SetBytes(l2PredeployNamespaceEcosystemAndIP.Bytes())
...
func AddressToCodeNamespace(addr common.Address) (common.Address, error) {
	...
	bigAddress := new(big.Int).SetBytes(append(addr[0:2], addr[18:]...))
...
func IsL2DevPredeploy(addr common.Address) bool {
	return bytes.Equal(addr[0:2], []byte{0x42, 0x00}) || bytes.Equal(addr[0:2], []byte{0x19, 0x00}) || bytes.Equal(addr[0:2], []byte{0x88, 0x00})
}
