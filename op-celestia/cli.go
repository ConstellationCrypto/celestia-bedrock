...
func ReadCLIConfigFromEnv(envPrefix string) CLIConfig {
	...
	//caldera does not tolerate *DA*_AUTH_TOKEN
	if value := os.Getenv(envPrefix + "_" + "AUTH_TOKEN"); value != "" {
		result.AuthToken = value
	}
	...
	if value := os.Getenv(envPrefix + "_" + "NAMESPACE"); value != "" {
		result.Namespace = value
	}
	...
	if value := os.Getenv(envPrefix + "_" + "S3_BUCKET"); value != "" {
		result.S3Bucket = value
	}

	if value := os.Getenv(envPrefix + "_" + "S3_REGION"); value != "" {
		result.S3Region = value
	}

	if value := os.Getenv(envPrefix + "_" + "CELESTIA_TIMEOUT"); value != "" {
